...
cv2.namedWindow('testitem')
cv2.createTrackbar('minval','image',0,255,nothing)
cv2.createTrackbar('maxval','image',0,255,nothing)


contours, hierarchy = cv2.findContours(edges, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE) #近似、找輪廓方法
print(len(contours))
draw = cv2.drawContours(img1, contours, -1, (255, 0, 0), 10) 
...
plt.subplot(133), plt.imshow(cv2.cvtColor(draw, cv2.COLOR_BGR2RGB)), plt.title('countour image')
plt.show()
